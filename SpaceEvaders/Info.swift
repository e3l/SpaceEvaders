...
class Info {
    ...
    func addPause(x: CGFloat, y: CGFloat) {
        let alien = Alien(x: -650, y: -5000, startAtTop: true)
        alien.addTo(info)
//        alien.setDisabled()
        Alien(x: 150, y: 200, startAtTop: true).addTo(info)
        Rocket(x: -550, y: 250).addTo(info)
        let pw = Powerup(x: x - 350, y: -200).addTo(info)
        pw.alpha = 1

        addText("Avoid the aliens", size: 80, x: 200, y: 100)
        addText("Spaceship follows finger", size: 80, x: -575, y: 100)
        addText("CREDITS", size: 80, x: x - 300, y: y - 550)
        addText("HOW TO PLAY", size: 130, x: 0, y: y - 400)
        addText("↑ this laser kills aliens", size: 60, x: -650, y: -100)
